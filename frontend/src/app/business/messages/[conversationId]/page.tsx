...
export default function BusinessConversationPage({
  ...
  const { data: meta, isLoading: metaLoading } = useQuery({
    queryKey: ['business-conversation-meta', conversationId],
    queryFn: async () => {
  ...
  const { data: messagesData, isLoading: messagesLoading } = useQuery<{ messages: Message[] }>({
    queryKey: ['business-messages', conversationId],
    queryFn: async () => {
  ...
  const sendMessage = useMutation({
    mutationFn: async (content: string) => {
      const res = await api.post(apiRoutes.messaging.messages(conversationId), { content });
      return res.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['business-messages', conversationId] });
  ...
  return (
    <div className="flex flex-col gap-6">
      <DashboardHero
        eyebrow="Business conversation"
        title={metaLoading ? 'Opening conversation...' : `Chat with ${meta?.participantName ?? 'customer'}`}
      ...
      >
        <DashboardHeroPill
          ...
          note="Continue customer conversations without leaving the business workspace."
        />
      </DashboardHero>
      ...
      <div className="flex h-[calc(100vh-12rem)] flex-col rounded-[24px] border border-border/70 bg-card shadow-[0_22px_70px_-48px_rgba(15,37,64,0.68)]">
        <div className="flex items-center gap-3 border-b px-4 py-3">
          <Button
            ...
            size="icon-sm"
            nativeButton={false}
            render={<Link href={appRoutes.business.messages} />}
          >
            <ArrowLeft />
